...
import com.example.demo.file.constrollers.RequestThumb;
...
public class ThumbnailService {
    ...
    public String getThumbPath(Long seq, int width, int height, boolean crop) {
        ...
        if (seq != null && seq > 0L) { // 직접 업로드한 파일 기준
            ...
            thumbPath = basePath + String.format(
                    "/%s/%s_%s_%s_%s%s", folder, width, height, crop, seq, Objects.requireNonNullElse(item.getExtension(), "")
            );
        }
        return thumbPath;
    }
